package fuck.system.enigma.ui.decoder
...
class DecoderFragment : Fragment()
{
    ...
    private lateinit var buttonDecode: Button

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        val view = inflater.inflate(R.layout.fragment_decoder, container, false)
        viewModel = ViewModelProvider(this)[DecoderViewModel::class.java]

        scrollView = view.findViewById(R.id.decoder_scroll_inputs)
        inputText = view.findViewById(R.id.decoder_edit_input_text)
        inputKey = view.findViewById(R.id.decoder_edit_decryption_key)
        spinnerMode = view.findViewById(R.id.decoder_spinner_cipher_mode)
        checkboxBase64 = view.findViewById(R.id.decoder_checkbox_base64)
        checkboxUserIv = view.findViewById(R.id.decoder_checkbox_iv_manual)
        blockIv = view.findViewById(R.id.decoder_group_iv)
        inputIv = view.findViewById(R.id.decoder_edit_iv_input)
        outputText = view.findViewById(R.id.decoder_text_output)
        buttonDecode = view.findViewById(R.id.decoder_button_decode)
        buttonCopy = view.findViewById(R.id.decoder_button_copy_output)
        resultGroup = view.findViewById(R.id.decoder_group_output)
        resultLabel = view.findViewById(R.id.decoder_text_output_label)
        buttonShare = view.findViewById(R.id.decoder_button_share_output)
        buttonHide = view.findViewById(R.id.decoder_button_hide_output)
    ...
    private fun setupListeners() {
        ...
        buttonDecode.setOnClickListener { onDecodeClicked() }
    ...
    private fun onShareClicked() {
        ...
        val errorPrefix = getString(R.string.decoder_message_error, "").removeSuffix("")
        ...
        val chooser = android.content.Intent.createChooser(intent, getString(R.string.decoder_message_share_via))
    ...
    private fun onDecodeClicked()
    {
        ...
        if (key.isEmpty() || text.isEmpty()) {
            Toast.makeText(requireContext(), getString(R.string.toast_text_and_key_required), Toast.LENGTH_SHORT).show()
            return
        }
        ...
        val cipher = Blowfish().apply {
            ...
            setEmbedIv(!userIv) // твоя логика: если пользователь НЕ вводит IV → он встроен
            setIv(if (userIv) ivHex?.let { hexToByteArray(it) } else null)
        }

        try {
            val result = cipher.decode(text, key)
            viewModel.result.value = result
            viewModel.isResultVisible.value = true
        } catch (e: Exception) {
            viewModel.result.value = getString(R.string.decoder_message_error, e.message)
        }
    }

    private fun onCopyClicked() {
        val clipboard = requireContext().getSystemService(android.content.ClipboardManager::class.java)
        val clip = android.content.ClipData.newPlainText(getString(R.string.decoder_message_clipboard), outputText.text)
    ...
    private fun hexToByteArray(hex: String): ByteArray {
        ...
        require(clean.length % 2 == 0) { getString(R.string.decoder_message_invalid_iv_hex) }
